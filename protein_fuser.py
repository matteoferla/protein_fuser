    ...
    def align(self, fore, aft, cycle=0):
        ...
        for i in ranger:
            ...
            if scores[i] < 100:
                ...
                if cycle < 6:
                    align(self, fore, aft, cycle=cycle + 1)
                else:
...
class Model:
    def __init__(self, mode, chain, code, x, y, start=1, end=None, seq=None, length=None, workshop=None, tier=0, metadata=None, similars=None, subtender=None):
        """
    ...
    def resi_list(self):
        return sorted(set([int(at.resi) for at in pymol.cmd.get_model(m.name).atom]))
    ...
    def load(self):
        ...
        if self.seq is None:
            self.seq = ''.join(pymol.cmd.get_fastastr(f'{self.name} and chain {self.chain} and not hetatm').split('\n')[1:])
        return self
    ...
    def shift_to_true_Cterminus(self):
        ...
        while coords is None:
            ...
            l -= 1
            if l < 0:
                raise ValueError(f'Trimming missing residues went haywire! start={self.x}/{self.start} end={self.y}/{self.end}. Died at {self.sele(l)}')
        self.end = l + 1
